...
var app = (function () {
    ...
    function create_if_block(ctx) {
    	let div;
    	let t0_value = /*hoverState*/ ctx[2].name + "";
    	let t0;
    	let t1;
    	let t2_value = /*hoverState*/ ctx[2].value.toLocaleString() + "";
    	let t2;
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div, file, 144, 2, 4016);
    		},
    		...
    		p: function update(ctx, dirty) {
    			if (dirty & /*hoverState*/ 4 && t0_value !== (t0_value = /*hoverState*/ ctx[2].name + "")) set_data_dev(t0, t0_value);
    			if (dirty & /*hoverState*/ 4 && t2_value !== (t2_value = /*hoverState*/ ctx[2].value.toLocaleString() + "")) set_data_dev(t2, t2_value);
    		},
    ...
    function create_fragment(ctx) {
    	...
    	let mounted;
    	let dispose;
    	let if_block = /*hoverState*/ ctx[2] && create_if_block(ctx);
    	...
    	const block = {
    		c: function create() {
    			...
    			div0 = element("div");
    			h1 = element("h1");
    			h1.textContent = "Median Household Income Distribution";
    			t1 = space();
    			div1 = element("div");
    			label = element("label");
    			t2 = text$1(/*sliderLabel*/ ctx[1]);
    			t3 = space();
    			input = element("input");
    			t4 = space();
    			if (if_block) if_block.c();
    			add_location(h1, file, 131, 4, 3742);
    			attr_dev(div0, "class", "title-container");
    			add_location(div0, file, 130, 2, 3708);
    			attr_dev(label, "for", "slider");
    			add_location(label, file, 134, 4, 3825);
    			attr_dev(input, "id", "slider");
    			attr_dev(input, "type", "range");
    			attr_dev(input, "min", "1984");
    			attr_dev(input, "max", "2018");
    			add_location(input, file, 135, 4, 3871);
    			attr_dev(div1, "class", "overlay");
    			add_location(div1, file, 133, 2, 3799);
    			add_location(main, file, 129, 0, 3699);
    		},
    		...
    		m: function mount(target, anchor) {
    			...
    			set_input_value(input, /*sliderYear*/ ctx[0]);
    			append_dev(main, t4);
    		...
    		p: function update(ctx, [dirty]) {
    			if (dirty & /*sliderLabel*/ 2) set_data_dev(t2, /*sliderLabel*/ ctx[1]);

    			if (dirty & /*sliderYear*/ 1) {
    				set_input_value(input, /*sliderYear*/ ctx[0]);
    			}
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	onMount(async () => {
    		$$invalidate(3, svg = select("#map").append("svg").attr("width", 960).attr("height", 600));
    		const projection = albersUsa().translate([480, 300]).scale(1000);
    		const path = index$2().projection(projection);
    		const geoData = await json("Simple.geojson");
    		const nestedData = await json("nested_data.json");
    		$$invalidate(4, allData = nestedData);
    		...
    		}).attr("stroke", "white").attr("stroke-width", "1.5").on("mouseover", (event, d) => {
    			$$invalidate(2, hoverState = {
    				name: d.properties.NAME,
    				value: allData[sliderYear][d.properties.NAME] || 0
    			});

    			select(event.currentTarget).style("fill", "orange");
    		}).on("mouseout", (event, d) => {
    			$$invalidate(2, hoverState = null);

    			select(event.currentTarget).style("fill", d => {
    				const stateName = d.properties.NAME;
    		...
    		const legendSvg = svg.append("g").attr("transform", `translate(${(svg.attr("width") - legendWidth) / 2},${svg.attr("height") - legendMargin.bottom - legendHeight})`);
    		const legendScale = linear().domain([0, .8]).range([0, legendWidth]);
    		const legendAxis = axisBottom(legendScale).ticks(0).tickSize(0).tickFormat("");
    		legendSvg.append("g").attr("class", "legend-axis").call(legendAxis).select(".domain").remove();
    		...
    		legendSvg.append("rect").attr("width", legendWidth).attr("height", legendHeight).style("fill", "url(#gradient)");
    		legendSvg.append("text").attr("class", "legend-label").attr("x", 0).attr("y", -5).attr("text-anchor", "start").text("Lower Income (Relative)");
    		legendSvg.append("text").attr("class", "legend-label").attr("x", legendWidth).attr("y", -5).attr("text-anchor", "end").text("Higher Income (Relative)");
    	});
    	...
    	function input_change_input_handler() {
    		sliderYear = to_number(this.value);
    		$$invalidate(0, sliderYear);
    	}
    	...
    	$$self.$inject_state = $$props => {
    		if ('sliderYear' in $$props) $$invalidate(0, sliderYear = $$props.sliderYear);
    		if ('sliderLabel' in $$props) $$invalidate(1, sliderLabel = $$props.sliderLabel);
    	...
    	$$self.$$.update = () => {
    		if ($$self.$$.dirty & /*sliderYear*/ 1) {
    			$$invalidate(1, sliderLabel = `Year: ${sliderYear}`);
    		}

    		if ($$self.$$.dirty & /*svg, allData, sliderYear*/ 25) {
    			{
    	...
    	return [sliderYear, sliderLabel, hoverState, svg, allData, input_change_input_handler];
    }
